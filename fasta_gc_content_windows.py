...
# gc content of fasta in windows of 100 bp
python fasta_gc_content_windows.py --input example.fasta --window 100

"""

description = """
Calculate GC content in windows for individual sequences in a fasta. Gzip compressed files can be used as input.
"""
# argparse
...
for record in dat:
    ...
    for i in range(0, len(sequence), args.window):
        ...
        gc_content = round(seq.gc_content(chunk), 2)
        line = name + "\t" + str(start) + "\t" + str(end) + "\t" + str(gc_content)
